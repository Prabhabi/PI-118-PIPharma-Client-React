...
import { styles } from "./../../../../../components/MasterTableUpdate/Property Master/PropertyMaster";
...
export default function InvoiceCreateDialogFormS({
  ...
  const [pdfOpened, setPdfOpened] = useState(false); // 🛑 Lock to prevent double open
  const [allRequestOrder, setAllRequestOrder] = useState([]);
  ...
  const schema = Yup.object().shape({
    selectedCustomer: showCustomerFields
      ? Yup.string().nullable()
    ...
    if (data1) {
      ...
      const arr = {
        ...
        EntityID: quoSection
          ...
          : data.selectedCustomer || null, // For "Without Quotation"
        BillEntityType: "COMP",
        ...
        RequestOrderMasterID: data.selectOrderRequest || null,
        ...
        ReceiptProductModelList: data.items.map((item) => ({
          ...
          BatchNo: item.child || null,

          CGSTP: (item.CGSTP || 0).toString(),
          SGSTP: (item.SGSTP || 0).toString(),
          Unit: null,
          Rate: item.Price,
          DISP: item.discount,
          Amount: item.result,
      ...
      try {
        ...
        if (responseData && !openEmailButton) {
          setRow(responseData);
          setTriggerPrint(true); // this will trigger the PDF
  ...
  //  =============================================================================
  const fetchRequestOrderFn = async (customer) => {
    const res = await axios.get(
      `${process.env.REACT_APP_URL}/api/getRequestOrderEntity?EntityID=${customer}`,
      {
        headers: {
    ...
    setAllRequestOrder(res.data.data);
  };
  // ======================================== update result =========================
  ...
  // const moneyReciptList = allRequestOrder || [];
  ...
  useEffect(() => {
    if (pdfReady) {
      ...
      setPdfReady(false);
      setPdfOpened(false); // allow for future PDF generations
    }
  }, [pdfReady]);
  ...
  const itemOptions = allItems.map((item) => ({
    value: item.ProductModelID,
    label: ` ${item.ModelNumber} ${item.Length !== "null" ? `${item.Length}` : ""}`,
  ...
  // PDF generation control states (pattern from QuotationFormOld.jsx)
  const [pdfTriggerCount, setPdfTriggerCount] = useState(0); // Counter for PDF triggers
  const pdfBlockRef = useRef(false); // Block duplicate PDF triggers
  // ...existing code...
  return (
    <Box padding={3}>
          ...
          <Grid container spacing={2}>
                          ...
                          render={({ field }) => (
                            <SelectComponent
                              ...
                              onChange={(selectedOption) => {
                                ...
                                fetchRequestOrderFn(selectedOption.value);
                              }}
                              styles={{
                                marginRight: "1rem",
              ...
              {fields.map((field, index) => (
                ...
                >
                  <InvoiceRow
                    ...
                    mrpSetFn={(value) => setValue(`items.${index}.MRP`, value)}
                  />
                  <IconButton
                    aria-label="delete"
                    color="error"
            ...
            <Grid container spacing={2} sx={{ mt: 2 }}>
              <Grid
                container
                spacing={2}
                sx={{ mt: 2, ml: 1, justifyContent: "center" }}
              >
                <Grid item xs={6} sm={2}>
                  <Controller
                    name="NetTotalAmount"
              ...
              <Box sx={{ width: "100%" }}>
                <InvoiceSalesPaymentForm
                  ...
                  moneyReciptList={allRequestOrder}
                  allRequestOrder={allRequestOrder}
                />
              </Box>
      ...
      {/* {triggerPrint && row && !pdfOpened && (
        <BlobProvider document={<InvoiceSalesCreatepdfNew {...row} />}>
          {({ url, loading }) => {
            if (url && !loading && !pdfOpened) {
              setPdfOpened(true); // 🔐 Lock it
              setTriggerPrint(false); // reset trigger
              setPdfReady(true); // 🔔 start background fetch
