...
const AdvncedORderEmailPdfDirect = (data, emailFormData, emailMszFn) => {
  console.log("data121", emailFormData);

  const token = Cookies.get("token");
  const sanctumToken = `Bearer ${token.replace(/"/g, "")}`; // Add token logic here  console.log("data", data);
  if (!data) return;
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; color: #000; padding: 5px; border: 1px solid #000; height: 100%; box-sizing: border-box; font-size: 10px;">
      <div style="text-align: center; margin-bottom: 10px;">
        ...
        <h1 style="margin: 0; font-size: 22px;">${BillCode === "QUO" ? "Quotation" : "Advance Order"}</h1>
      </div>
...
export default AdvncedORderEmailPdfDirect;
