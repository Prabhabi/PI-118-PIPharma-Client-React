...
const CustomTooltip = ({ active, payload }) => {
  if (active && payload && payload.length) {
    ...
    return (
      ...
      >
        <p className="label">{`Month: ${monthShortNames[data.MonthName]}`}</p>
...
export default function QuotationGraphDetailsSubd({
  ...
  const fetchYearWiseData = async (year) => {
    const res = await axios.get(
      `${process.env.REACT_APP_URL}/api/getEntityWiseMonthlyBooking?EntityType=SUBD&BillCode=ADVB&BillYear=${year}`,
      {
        headers: {
      ...
      <Stack direction="row" spacing={2}>
        <Box flex={1}>
          <Stepper
            ...
            sx={{
              "& .MuiStepLabel-root .Mui-active": { color: "blue" },
              "& .MuiStepLabel-root .Mui-completed": { color: "blue" },
            }}
          >
        ...
        >
          ...
          >
            <Typography variant="h6">Select Sub Dealer</Typography>
            <Box sx={{ width: "15rem" }}>
              <Select
                value={selectedName}
